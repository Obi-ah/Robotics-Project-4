...
class PosePublisher(Node):
   ...
   def publish_pose(self):
       ...
       if results:
           ...
           X = distance + 0.09
           Y = -distance * (center[0] - ppx) / fx +0.005
           Z = -distance * (center[1] - ppy) / fy + 0.027

           # X2, Y2, Z = rs.rs2_deproject_pixel_to_point(depth_intrinsics, (center), distance)

           t_vec = np.array([X, Y, Z])
           #t_vec = t_vec * 10  # change to centimeters

           # Calculate euler angles
           M = self.detector.detection_pose(tag, (fx, fy, ppx, ppy))[
               0]  # compare translation vector to prevailing  one
